...
export class ClientSideRouter {
  ...
  get query() {
    return ClientSideRouter.parseQuery(window.location.search);
  }

  set query(newQuery) {
    const newUrl = ClientSideRouter.getUrl(newQuery, this.#baseUrl);
    this.push(newUrl);
  }
  ...
  static getUrl = (newQuery, baseUrl = "") => {
    const currentQuery = ClientSideRouter.parseQuery();
    ...
    const queryString = ClientSideRouter.stringifyQuery(updatedQuery);
